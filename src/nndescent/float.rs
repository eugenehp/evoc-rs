//! Float NN-descent (port of `evoc.float_nndescent`, sorted update path).

use crate::fast_cosine::fast_cosine;
use crate::heap::{
...
const EPS: f32 = 1e-8;
pub fn float_random_projection_split(
    data: &Array2<f32>,
    indices: &[i32],
    rng_state: &mut [i64; 3],
) -> (Vec<i32>, Vec<i32>) {
    ...
    let mut hyperplane_norm = hyperplane_norm.sqrt();
    ...
    let indices_left = temp_left[..n_left].to_vec();
    let indices_right = temp_right[..n_right].to_vec();
    (indices_left, indices_right)
}

pub fn make_float_tree(
    data: &Array2<f32>,
    indices: &[i32],
    ...
    if indices.len() > leaf_size && max_depth > 0 {
        let (left_indices, right_indices) = float_random_projection_split(data, indices, rng_state);
        make_float_tree(
            data,
            &left_indices,
            point_indices,
...
fn make_float_leaf_array_serial(
    data: &Array2<f32>,
    rng_state: &mut [i64; 3],
    leaf_size: usize,
    max_depth: i64,
...
pub fn make_float_forest(
    data: &Array2<f32>,
    rng_states: &Array2<i64>,
    leaf_size: usize,
    max_depth: i64,
) -> Vec<Array2<i32>> {
    let mut out = Vec::with_capacity(rng_states.nrows());
    for i in 0..rng_states.nrows() {
        let mut state = [rng_states[[i, 0]], rng_states[[i, 1]], rng_states[[i, 2]]];
        out.push(make_float_leaf_array_serial(
            data, &mut state, leaf_size, max_depth,
        ));
    }
    out
}

fn generate_leaf_updates_float(
        ...
        for r in 0..rows_per_thread {
            ...
            for i in 0..max_leaf {
                ...
                for j in (i + 1)..max_leaf {
                    ...
                    let d = fast_cosine(data_p, data.row(q as usize));
                    let max_threshold =
    ...
    (0..n_points).into_par_iter().for_each(|i| {
        ...
        unsafe {
            ...
            for _ in 0..to_fill {
                ...
                let d = fast_cosine(data.row(idx), data.row(i));
                flagged_heap_push(
...
/// Unsorted NN-descent update generation (Python: `generate_graph_update_array_float`).
pub fn generate_graph_update_array_float(
    ...
    let counts_ptr = SendMutPtr::new(n_updates_per_thread.as_mut_ptr());
    ...
    (0..n_threads).into_par_iter().for_each(|t| {
        ...
        for r in 0..rows_per_thread {
            ...
            if i >= block_size || idx >= max_updates {
                break;
            }

            let mut new_start = 0usize;
            while new_start < max_new_candidates && idx < max_updates {
                let new_end = (new_start + working_set_size).min(max_new_candidates);
                ...
                for j in new_start..new_end {
                    ...
                    for k in (j + 1)..new_end {
                        ...
                        let d = fast_cosine(data_p, data.row(q as usize));
                        let max_threshold = dist_thresh_p.max(dist_thresholds[q as usize]);
                        if d <= max_threshold {
                            unsafe {
                                let base = update_ptr.as_ptr().add(t * update_stride + idx * 3);
                                *base = p as f32;
                                *base.add(1) = q as f32;
                                *base.add(2) = d;
                            }
                    ...
                    for k in new_end..max_new_candidates {
                        if idx >= max_updates {
                            break;
                        }
                        let q = new_candidate_block[[i, k]];
                    ...
                    while old_start < max_old_candidates && idx < max_updates {
                        let old_end = (old_start + working_set_size).min(max_old_candidates);
        ...
        unsafe {
            *counts_ptr.as_ptr().add(t) = idx as i32;
        }
    });
}

/// Generate graph updates bucketed by target vertex block.
pub fn generate_sorted_graph_update_array_float(
    update_array: &mut Array3<f32>,
...
/// Approximate k-NN via NN-descent (unsorted updates). Returns heap distances (negative cosine).
pub fn nn_descent_float(
    data: &Array2<f32>,
    n_neighbors: usize,
    rng_state: &mut [i64; 3],
    ...
    use crate::heap::{apply_graph_update_array, build_candidates, deheap_sort, make_heap};

    let n_threads = rayon::current_num_threads();
    let mut current_graph = make_heap(data.nrows(), n_neighbors);

    if let Some(leaf) = leaf_array {
        init_rp_tree_float(data, &mut current_graph, leaf, n_threads);
    }
    init_random_float(n_neighbors, data, &mut current_graph, rng_state);

    let n_vertices = data.nrows();
    // Match Python: block_size = 65536 // n_threads (also sizes the update buffer).
    let mut block_size = 65536 / n_threads.max(1);
    ...
    let mut update_array = Array3::zeros((n_threads, max_updates_per_thread, 3));
    let mut n_updates_per_thread = vec![0i32; n_threads];
...
pub fn nn_descent_float_sorted(
    data: &Array2<f32>,
    n_neighbors: usize,
    rng_state: &mut [i64; 3],
    max_candidates: usize,
